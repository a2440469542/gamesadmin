    ...
    <div class="user-container">
      <div class="btn-group">
        <div class="search">
          ...
          <label>渠道：</label>
          <ChannelSelect v-model="userParam.cid" @change="handleChannelFilter"></ChannelSelect>
          <label>提款：</label>
          <el-select
            style="margin:0 6px"
      ...
      <el-dialog
        :title="'生成试玩账号'"
        :visible.sync="isShowBot"
        width="30%"
        :before-close="handleClose"
      >
        <el-form ref="botForm" :model="botParam" label-position="left">
      ...
      <el-dialog
        ...
        :before-close="handleClose"
      >
        <el-form ref="dataForm" :model="userUpdate" label-position="left">
      ...
      <el-dialog
        ...
        :before-close="handleClose"
      >
        <el-form ref="dataForm" :model="recahrge" label-position="left">
      ...
      <el-dialog
        :title="'充值积分'"
        :visible.sync="re_score_dialog"
        width="30%"
        :before-close="handleCloseScore"
      >
        <el-form ref="dataForm" :model="re_score" label-position="left">
      ...
      >
        ...
        >
          ...
          <el-table-column align="center" label="宝箱领取金额">
            <template slot-scope="scope">{{ scope.row.box_money }}</template>
          </el-table-column>
        </el-table>
        <span slot="footer" class="dialog-footer">
      ...
      <el-dialog
        :title="'绑定上级'"
        :visible.sync="bindParentDialog"
        width="30%"
        :before-close="handleBindParentDialogClose"
      >
        <el-form ref="bindParentForm" :model="bindParentModel" label-position="left">
      ...
      <el-dialog
        :title="'冻结/解冻账户'"
        :visible.sync="bindUserDialog"
        width="30%"
        :before-close="handleBindUserDialogClose"
      >
        <el-form :model="bind_user" label-position="left">
          <el-form-item label="冻结/解冻账户">
            <el-radio-group v-model="bind_user.is_bind" @change="user_is_bind()">
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="handlebindUserSubmit">确 定</el-button>
        </span>
      </el-dialog>

      <el-dialog
        :title="'冻结/解冻下级账户'"
        :visible.sync="bindUserChildDialog"
        width="30%"
        :before-close="handleBindUserChildDialogClose"
      >
        <el-form :model="bind_user_child" label-position="left">
          <el-form-item label="冻结/解冻下级账户">
            <el-radio-group v-model="bind_user_child.is_bind" @change="user_child_is_bind()">
              <el-radio :label="1">冻结</el-radio>
              <el-radio :label="0">解冻</el-radio>
            </el-radio-group>
          </el-form-item>
        ...
        <span slot="footer" class="dialog-footer">
          <el-button @click="bindUserChildDialog=false">取 消</el-button>
          <el-button type="primary" @click="handlebindUserChildSubmit">确 定</el-button>
        </span>
      </el-dialog>
      ...
      >
        ...
        <el-table-column label="上上上级ID" align="center" width="100">
          <template slot-scope="scope">{{ scope.row.pppid }}</template>
        </el-table-column>
        <el-table-column label="邀请人数" align="center" width="80">
          <template slot-scope="scope">
            <el-link
              type="primary"
              @click="showInviteDialog(scope.$index, scope.row, 1)"
            >{{ scope.row.child_num }}</el-link>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" prop="created_at" label="操作" fixed="right" width="210">
          <template slot-scope="scope">
            <el-button size="mini" @click="recharge(scope.$index, scope.row)">充值</el-button>
            <el-button size="mini" @click="re_score_btn(scope.$index, scope.row)">充值积分</el-button>
            <el-button size="mini" @click="showBindParentDialog(scope.$index, scope.row)">绑定上级</el-button>
            <el-button size="mini" @click="showBindUser(scope.$index, scope.row)">冻结/解冻账户</el-button>
            <!-- <el-button size="mini" @click="showBindChild(scope.$index, scope.row)">冻结/解冻下级账户</el-button> -->
            <el-button size="mini" type="primary" @click="showUnlockUserAmount(scope.$index, scope.row)" v-if="scope.row.lock_money">解冻余额</el-button>
            <el-button size="mini" type="warning" @click="showBindUserAmount(scope.$index, scope.row)" v-else>冻结余额</el-button>

            <el-button size="mini" @click="BindChilendAmount(scope.$index, scope.row)">冻结下级余额</el-button>
...
export default {
  ...
  methods: {
    ...
    showUnlockUserAmount(index, row) {
      this.$confirm("此操作会将用户余额解冻, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
        center: true
        ...
        .then(() => {
          const params = {
            cid: row.cid,
            uid: row.uid,
            money: row.lock_money
          };
          api_unlock_money(params).then(response => {
            this.fetchData();
            this.$message({
              type: "success",
              message: "解冻成功!"
    ...
    UnlockChilendAmount(index, row) {
      this.$confirm("此操作会将用户下级余额解冻, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
        center: true
        ...
        .then(() => {
          ...
          api_unlock_chilend_money(params).then(response => {
            this.fetchData();
            this.$message({
              type: "success",
              message: "解冻成功!"
    ...
    handle_submit() {
      reSCoreMoney(this.re_score).then(res => {
        if (res.code === 0) {
    ...
    showBindUser(index, row) {
      this.bindUserDialog = true;
      this.bind_user = {};
      this.bind_user.is_bind = row.is_bind;
      this.bind_user.uid = row.uid;
      this.bind_user.cid = row.cid;
    },
    handleBindUserDialogClose() {
      this.bindUserDialog = false;
    },
    handlebindUserSubmit() {
      userBindUser(this.bind_user).then(res => {
        if (res.code == 0) {
          this.fetchData();
          this.bindUserDialog = false;
          this.$message({
    ...
    showBindChild(index, row) {
      this.bindUserChildDialog = true;
      this.bind_user_child = {};
      this.bind_user_child.is_bind = row.is_bind;
      this.bind_user_child.uid = row.uid;
      this.bind_user_child.cid = row.cid;
    },
    handleBindUserChildDialogClose() {
      this.bindUserChildDialog = false;
    },
    handlebindUserChildSubmit() {
      userbind_child_user(this.bind_user_child).then(res => {
        if (res.code == 0) {
          this.fetchData();
          this.bindUserChildDialog = false;
          this.$message({
